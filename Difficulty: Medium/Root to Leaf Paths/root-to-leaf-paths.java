...
class Solution {
    ...
    private static void dfs(Node node, ArrayList<Integer> currentPath, ArrayList<ArrayList<Integer>> result) {
        ...
        // If it's a leaf node, add a copy of current path to result
        if (node.left == null && node.right == null) {
            result.add(new ArrayList<>(currentPath));
        } else {
            dfs(node.left, currentPath, result);   // Traverse left first
            dfs(node.right, currentPath, result);  // Then right
        }
